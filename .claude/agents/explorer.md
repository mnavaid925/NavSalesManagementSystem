...
Repo shape:
  ...
  - `apps/` — `core` (Tenant + TenantMiddleware + `navigation.py` (MODULE_CATALOG 0-20 + LIVE_LINKS) + AuditLog +
    context processors), `accounts` (User/Role/UserInvite/UserPreference + `backends.py` email-or-username + auth +
    user/role/profile mgmt), `tenants` (Module 0 — Tenant & Subscription), `projects` (Project/Task/Meeting/Ticket/
    Invoice demo), `dashboard` (aggregation view, no models).
...
Given a task, find and report:
  ...
  - **Patterns to follow** so new code stays consistent — `apps/tenants` is the reference module; `apps/projects`
